...
<script src="<?= e(asset('plugins/jquery_ui/jquery-ui.1.12.1.min.js')) ?>"></script>
<script src="<?= e(asset('plugins/select2/js/select2.min.js')) ?>"></script>
<script src="<?= e(asset('plugins/datatables/js/jquery.dataTables.min.js')) ?>"></script>
<script src="<?= e(asset('plugins/datatables/js/datatables.init.js')) ?>"></script>
<script src="<?= e(asset('plugins/flatpickr/flatpickr.min.js')) ?>"></script>
<script src="<?= e(asset('plugins/dropzone/dropzone.min.js')) ?>"></script>
<script src="<?= e(asset('plugins/dropzone/dropzone_custom.js')) ?>"></script>
<script src="<?= e(asset('plugins/tinymce/tinymce.min.js')) ?>"></script>
<script src="<?= e(asset('plugins/prism/prism.js')) ?>"></script>
<script src="<?= e(asset('plugins/jquery-repeater/jquery.repeater.js')) ?>"></script>
...
(function(){
    ...
    function openSidebar(){
        body.classList.add('ent-sidebar-open');
        if(hamburger) hamburger.querySelector('i').className = 'bi bi-x-lg';
    }
    function closeSidebar(){
        body.classList.remove('ent-sidebar-open');
        if(hamburger) hamburger.querySelector('i').className = 'bi bi-list';
    }

    /* On page load: ensure sidebar is closed on mobile */
    if(isMobile()){ closeSidebar(); }

    if(hamburger){
        hamburger.addEventListener('click', function(e){
            e.stopPropagation();
            body.classList.contains('ent-sidebar-open') ? closeSidebar() : openSidebar();
        });
    }
    if(overlay){
    ...
    /* Resize: close sidebar when resizing to desktop */
    window.addEventListener('resize', function(){
        if(!isMobile()){ closeSidebar(); }
    });

    /* ── Notification bell dropdown ── */
    var bellBtn  = document.getElementById('deskBellBtn');
    var bellDrop = document.getElementById('deskNotifDropdown');
    if (bellBtn && bellDrop) {
        ...
        document.addEventListener('click', function(e){
            if (!bellDrop.contains(e.target) && e.target !== bellBtn) {
    ...
    if (typeof Swal !== 'undefined') {
        ...
        /* ── Override window.confirm with SweetAlert2 ──
           Works for ALL onclick="return confirm(...)" and onsubmit="return confirm(...)"
        ...
        function sweetConfirm(message, onConfirm) {
            Swal.fire({
                ...
                reverseButtons: true,
                focusCancel: true,
                customClass: { popup: 'ent-swal-popup' }
