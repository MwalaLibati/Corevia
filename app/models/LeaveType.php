...
class LeaveType extends Model
{
    ...
    public function active(): array
    {
        ...
        $stmt = $this->db->prepare(
            'SELECT * FROM leave_types WHERE is_active = 1' . ($cid > 0 ? ' AND company_id = :cid' : '') . ' ORDER BY name ASC'
        );
    ...
    public function search(string $keyword): array
    {
        ...
        $stmt = $this->db->prepare(
            "SELECT * FROM leave_types WHERE (name LIKE :keyword OR code LIKE :keyword)$and ORDER BY name ASC"
        );
    ...
    {
        $cid = Tenant::id();
        $sql = "SELECT id FROM leave_types WHERE {$column} = :value" . ($cid > 0 ? ' AND company_id = :cid' : '');
        $params = ['value' => $value];
        if ($cid > 0) {
            $params['cid'] = $cid;
        }
    ...
    {
        $cid = Tenant::id();
        $sql = "SELECT code FROM leave_types WHERE code REGEXP :pattern"
