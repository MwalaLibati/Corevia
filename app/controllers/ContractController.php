...
class ContractController extends Controller
{
    ...
    private function sendContractEmail(int $contractId): array
    {
        ...
        if ($template) {
            ...
            $signatureBody = trim((string)($template['signature_body'] ?? '')) ?: $tmplModel->professionalDefaultSignature();
            $footerBody = trim((string)($template['footer_body'] ?? '')) ?: $tmplModel->professionalDefaultFooter();
            $renderedBody = $tmplModel->renderBody($coverBody, $values)
                . '<div style="page-break-before:always"></div>' . $renderedBody
                . $tmplModel->renderBody($signatureBody, $values)
                . '<hr>' . $tmplModel->renderBody($footerBody, $values);
        }
